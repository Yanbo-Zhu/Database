...
一个 relation 中德 一个 column（attribute） 就是一个 predicate 

![](image/Pasted%20image%2020260121171258.png)

a,b,c 就是 一个 predicate 
...
## 1.8 γ: Grouping and Aggregation 

符号： γ（希腊字母 gamma）

作用： 表示 分组（grouping） 和 聚合（aggregation） 操作


$\gamma$


假设有一个表 `Took(MatNo, CNo, Score)`，想要按课程分组统计平均分：

**关系代数写法：** γ_{CNo; AVG(Score)}(Took)
...
- Student(MatNo, SName)
- Course(CNo, CName)
- Took(CNo -> Course, MatNo -> Student, Score)

select all correct statements on the following relational queries.

![[Pasted image 20251019161120.png]]


$\gamma_{count(*) -> cnt}$

下标 `{count(*) -> cnt}` 表示 **聚合函数和输出列名**
...
|  运算符  | 含义   | 类似于数学符号 | 结果为真（TRUE）的条件 |
| :---: | :--- | :-----: | :------------ |
| `AND` | “并且” |    ∧    | 只有当所有条件都为真时   |
| `OR`  | “或者” |    ∨    | 只要有一个条件为真时    |

![[Pasted image 20251019152635.png]]
